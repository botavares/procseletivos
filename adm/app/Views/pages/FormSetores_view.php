					...
					<div class="card">
						...
						<div class="card-body">
							<form id="formSetores" class="formAdmin form-horizontal col-md-12 col-lg-12" method="POST" action="<?php echo base_url()."/Setores/registrar"?>">
								...
								<hr>
								<div>
									<span class="text text-danger">Prezado agente, os assuntos precisam estar cadastrados para aparecer na relação abaixo. Clique <a class="blue bold font16" href="<?php echo base_url()."/Assuntos/formularioCadastro"?>">AQUI</a> aqui para Cadastrar</span>
								</div>
								<div class="form-group row">
									<label for="input-editgroupSetores" class="col-form-label col-sm-2">Assuntos pertinentes ao Setor</label>
									<div class="col-sm-10">
                                    <span class="text text-danger"><?php echo session()->getFlashdata('errors')['ds_assuntoSetores'] ?? ''?></span> 
										<?php 
											$totalRegistros = count($assuntos);
											$totalPorColuna = $totalRegistros / 3;
											$totalPorColuna = ceil($totalPorColuna);
											$contarRegistrosPorColuna = 1;
											foreach($assuntos as $valueAssuntos){
												if($contarRegistrosPorColuna > $totalPorColuna || $contarRegistrosPorColuna == 1){
													echo '</div><div class="form-group col-lg-4">';
												}
												if($assuntosSetores != null){
													if(in_array($valueAssuntos->pk_id_assunto, $assuntosSetores)){
														$checked = 'checked';
													}else{
														$checked = '';
													}
										...
										?>
												<div class="custom-check">
													<label>
														<input 
															type="checkbox" 
															id="<?php echo "input-editgroupSetores".$valueAssuntos->pk_id_assunto?>" 
															class="grupoSetores" 
															name="ds_assuntoSetores[]"
															value="<?php echo $valueAssuntos->pk_id_assunto?>" 
																	<?php echo set_radio('ds_assuntoSetores[]',$valueAssuntos->pk_id_assunto)?>
															<?php echo $checked?>
															><span style="margin-left:5px;"><?php echo $valueAssuntos->ds_assunto?></span>
														</label>
													</div>
													<?php
														if($contarRegistrosPorColuna > $totalPorColuna || $contarRegistrosPorColuna == 1){
